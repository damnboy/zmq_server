...
SIGUSR1 
    is reserved by Node.js to start the debugger. 
    It"s possible to install a listener but doing so will not stop the debugger from starting.
...
SIGHUP 
is generated on Windows when the console window is closed, and on other platforms under various similar conditions, see signal(7).
...
SIGINT 
from the terminal is supported on all platforms, 
...
on non-Windows platforms it can be listened on, but there is no way to send or generate it.
...
Note: Windows does not support sending signals, but Node.js offers some emulation with 
process.kill(), and ChildProcess.kill(). 
