...
var procutil = require('../../../submodules/stf/lib/util/procutil')
var lifecycle = require('../../../submodules/stf/lib/util/lifecycle')
var srv = require('../../../submodules/stf/lib/util/srv')
var zmqutil = require('../../../submodules/stf/lib/util/zmqutil')
...
module.exports = function(options) {
  ...
  client.trackDevices().then(function(tracker) {
    ...
    tracker.on('add', filterDevice(function(device) {
      ...
      function spawn() {
        ...
        return resolver.promise
          ...
          .catch(Promise.CancellationError, function() {
            //调用promise上的cancel方法后，触发此callback
            //
      ...
      //本应有sub上的DeviceRegisteredMessage消息触发
      flippedTracker.emit(device.id, 'register')
    ...
    sub.on('message', wirerouter()  
      .on(wire.DeviceRegisteredMessage, function(channel, message) {
        console.log('[PROVIDER GOT MESSAGE FROM PUBLISHER]' + message.serial)
  ...
  lifecycle.observe(function() {
    ...
    */
    ...
    return Promise.all(Object.keys(workers).map(function(serial) {
      //console.log(workers)
      //log.info('shutting down working processes')
...
var runtimeOpt = {
    ...
    ,fork: function(device, ports){
      return require('child_process').fork('./proc.js')
    }
    ,killTimeout: 5000
}

module.exports(runtimeOpt)