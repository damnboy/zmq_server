...
module.exports = function DeviceServiceFactory($http, socket /*,EnhanceDeviceService*/) {
  ...
  function Tracker($scope, options) {
    ...
    /*
      判断设备可用的逻辑
        data.present
          DeviceIntroductionMessage false
          DevicePresentMessage      true

        data.status
          DeviceIntroductionMessage message.status
        
        data.ready
          DeviceIntroductionMessage false
          DeviceReadyMessage        true

        data.owner
          DeviceIntroductionMessage  null
          DeviceReadyMessage         null
          JoinGroupMessage           message.owner
    ...
    */
    ...
    function changeListener(event) {
      //console.log('changeListener' + event)
      var device = get(event.data)
      if (device) {
        modify(device, event.data)
    ...
    scopedSocket.on('device.add', addListener)//DeviceIntroductionMessage
    scopedSocket.on('device.remove', changeListener)//DeviceAbsentMessage
